 ...
 * user to type in entries which specify a new child for the currently selected
 * xml element. Note that it is not possible to infer which attributes should be
 * present in a child, unless the element has other children which you can copy
 */
public class AddChildOptionPane extends JComponentHandler {
    ...
    public AddChildOptionPane(ObjectModelElement e, String m, int index) {
    ...
    String prompt = "add new child to " + getSelectedElement().getAttributeValue("name");
       
        //set up the event handling for the option pane
            int result = JOptionPane.showOptionDialog(null,
        prompt,
        "Create new child", // the title of the dialog window
	...
	case 2:
            ...
            case 3 : // enter
               ...
               AddChild ac = new AddChild(parser, selectedElement,newElement, index);
                ac.actionPerformed(new ActionEvent(this, 0, "addchildoptionpane"));
                break;
            case 4 : // cancel
    ...
    private void createAttributePane() {
            ...
            //Find the first other child of this element and use its attributes
            List kids = selectedElement.getChildren();
            ObjectModelElement sibling = null;
            if (kids != null && kids.size()> 0) sibling = (ObjectModelElement) kids.get(0);
            List attrs;
            if (sibling != null){ 
                    attrs = sibling.getAttributes();
                    //as this has a sibling, set the default value for the element name to that of the sibling
                    textField.setText(sibling.getName());
                    //but if there are no siblings, just default to the attributes of the element itself
            }else {
                    attrs = selectedElement.getAttributes();
                    textField.setText(selectedElement.getName());
                    } 
