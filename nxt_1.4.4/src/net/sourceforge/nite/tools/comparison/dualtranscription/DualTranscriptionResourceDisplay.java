...
public class DualTranscriptionResourceDisplay {
    ...
    */
    public DualTranscriptionResourceDisplay(String c, String o, String cf, String gs, String cs1, String cs2, String r1n, String r2n) {
	corpusname = c;
	observationname = o;
	configFilename = cf;
	guisettings = gs;
	...
	if (controlData.getCorpusType() == NMetaData.STANDOFF_CORPUS) {
		...
		nom.forceResourceLoad(r1name);
		nom.forceResourceLoad(r2name);
	...
	addTranscriptionView(0, 0, 540, 460, ntv1, config1, r2name);
	System.out.println("ntv2");
	addTranscriptionView(540, 0, 540, 460, ntv2, config2, r1name); //TODO: ntv2 
	//addOntology(540, 0, 240, 460);
	//addLegend(540, 460, 240, 100, r2name, r1name);
     ...
     */
    protected void setupTranscriptionView(int x, int y, int width, int height, NTranscriptionView ntv, NonSpanningCoderConfig config, String rname) {
	ntv=new NTranscriptionView();
	ntv.setClock(niteclock);
	JInternalFrame ntvFrame = new JInternalFrame("Transcription " + rname, true, false, true, true);
	...
	initTranscriptionViewSettings(ntv, config, rname);
	refreshTranscriptionView(ntv, config, rname);
    }    
    ...
    */
	...
	mvds = new MergedViewDisplayStrategy(ntv, roleName, abbrevAttr, attrName, annlay, annotatorattr, r2name, r1name);
	mvds.styletext(true);
     ...
     */
    public void refreshTranscriptionView(NTranscriptionView ntv, NECoderConfig config, String rname) {
	...
	//display segments
	String resstr = "";
	if (rname!=null && !rname.equals("") && controlData.getResourceAttributeName()!=null) {
	    resstr = ":$a@" + controlData.getResourceAttributeName() + "=\'"+rname+"'";
	}
	Iterator elemIt = search("($a " + config.getSegmentationElementName() + ")"+resstr).iterator();
    ...
    private void addTranscriptionView(int x, int y, int width, int height, NTranscriptionView ntv, NonSpanningCoderConfig config, String rname) {
	...
	//NLayer annlay = controlData.getLayerByName(config.getAnnotatorLayer());
	//ntv.setDisplayStrategy(elName, new MergedViewDisplayStrategy(ntv, roleName, abbrevAttr, attrName, annlay, annotatorattr, r2nname, r1name));
	...
	ntv.setAllowTranscriptSelect(true);
	ntv.setAllowAnnotationSelect(true);
	setupTranscriptionView(x, y, width, height, ntv, config, rname);
    }
    ...
    /*	private void addNavigation(int x, int y, int width, int height) {
      ...
      String[] names = {"all annotators", r2name, r1name};
    ...
    */ 
	...
	for (int i=0; i<args.length; i++) {
	    ...
	    } else if (flag.equals("-ann1-name") || flag.equals("-r1")) {
		i++; if (i>=args.length) { usage(); }
		r1name=args[i];
	    } else if (flag.equals("-ann2-name") || flag.equals("-r2")) {
		i++; if (i>=args.length) { usage(); }
		r2name=args[i];
	    } else {
		usage();
	...
	if (corpus == null || configfile == null || r2name == null || r1name == null) { System.out.println("Invalid arguments: " + args); usage(); }
		
	DualTranscriptionResourceDisplay dt = new DualTranscriptionResourceDisplay(corpus, observation, configfile, guisettings, corpussettings1, corpussettings2, r1name, r2name);
    }
	
    protected static void usage() {
	System.err.println("Usage: java DualTranscriptionResourceDisplay -c metadata-filename -o observation-name -cf config-filename -gs gui-settings -cs1 corpus-settings1 -cs2 corpus-settings2 -r1 resource1 -r2 resource2 \n\n    Note that the resource names should be at the segment level.");
	System.exit(0);
    }
    ...
    */
	...
	if (annname.equals(r2name)) {
	    return ann1elements;
	} else if (annname.equals(r1name)) {
	    return ann2elements;
	} else {
	    return annelements;
	}
